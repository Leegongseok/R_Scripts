...
text <- "예시 문장입니다. 형태소 분석을 수행합니다."
result <- mecab(text)

# 분석 결과 확인
print(result)
searchUrl="https://openapi.naver.com/v1/search/news.xml"
Client_ID="e1TecD7uDpyn1wMCw1Eo"
Client_Secret="TtilxK75ye"

#검색하고자하는 키워드를 한글인 경우 UTF-8로 인코딩 설정 
...
#XML 파일의 <item> 태크를 기준으로 데이터프레임으로 변환
...
nouns.df.softdesc=nouns.df[order(-nouns.df$Freq),]
nouns.df.softdesc

#워드 클라우드를 사용하여 빈도수의 따른 단어들을 시각화
wordcloud2(nouns.df.softdesc,size=1,rotateRatio = 0.5)

wordcloud2(nouns.df.softdesc,size=0.3,shape='star')