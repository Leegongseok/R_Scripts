...
#그래프 에니메이션
#막대그래프 에니메이션:분기별로 막대의 색상이 변경되는 에니메이션
...
anim=ggplot(df,aes(x=부서,y=매출,fill=분기))+
  ...
  labs(title="부서별분기별영업실적")+
  transition_states(분기)
#애니메이션 효과 및 실행 
animate(anim,width=300,height=200,duration=3,renderer=gifski_renderer(loop=3))

#에니메이션으로 ENCODING된 그래프를 GIF 형식의 이미지로 저장하는 방법

anim_save("분기별 부서별영업실적.gif",path = NULL)
anim_save("분기별 부서별영업실적.gif",path = "C:/Users/AISW-509-208/Pictures")
...
ggplot(data=iris,aes(x=Sepal.Length,y=Sepal.Width,fill=Species,color=Species))+
  ...
  labs(title="꽃받침크가와 종의 분포도",x="꽃받침길이",y="꽃받침너비")
...
anim_save("꽃받침 크기와 종의 분포도.gif",path = "C:/Users/AISW-509-208/Pictures")
#선그래프 에니메이션
#선형 그래프 그리기
month=c(1,2,3,4,5,6)
sales=c(3,3,5,5,7,4)
