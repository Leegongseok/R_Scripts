...
for(x in 1:iteration){
  #동전던지기(추출개수 1개이므로 벅원 또는 비복원 추출의 결과가 같다.)
  coin=sample(c("앞면","뒷면"),1,replace = T) #복원추출
  
  #동전이 앞면인 경우에 앞면횟수를 저장하는 변수 count에 1씩 증가
  if(coin =="앞면"){
    count =count+1
  }
  #누적비율 추가
...
#원주율 구하는 실험(시뮬레이션)
#한변의 길이가 사각형의 1/4의 원이 포함되어 있는 경우 
#변수 초기화
#반복 횟수
iteration <- 100
# 1/4원 안의 점의 개수를 저장
